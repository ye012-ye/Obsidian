...
*(⚠️ 图片缺失:源知识库原图已失效)*![](../../assets/110a23f58a590de9.png)

如上图，主题T有4个分区，群组中只有一个消费者，则该消费者将收到主题T1全部4个分区的消息。

*(⚠️ 图片缺失:源知识库原图已失效)*![](../../assets/5bf92da066b99760.png)

如上图，在群组中增加一个消费者2，那么每个消费者将分别从两个分区接收消息，上图中就表现为消费者1接收分区1和分区3的消息，消费者2接收分区2和分区4的消息。

*(⚠️ 图片缺失:源知识库原图已失效)*![](../../assets/edec17f797beb84c.png)

如上图，在群组中有4个消费者，那么每个消费者将分别从1个分区接收消息。

*(⚠️ 图片缺失:源知识库原图已失效)*![](../../assets/d299e5f0649eaa54.png)
...
![](../../assets/2f0320484e3a206c.png)

### 自定义序列化

代码见：

![](../../assets/987d18b5cf7b08a6.png)

代码中使用到了自定义序列化。

![](../../assets/697e978e1861b09d.png)

id的长度4个字节，字符串的长度描述4个字节， 字符串本身的长度nameSize个字节

![](../../assets/329d88d7daf19042.png)
...
![](../../assets/6f8cc491ca3caf6c.png)

### 系统分区器

在我们的代码中可以看到，生产者参数中是可以选择分区器的。

![](../../assets/7ac7aa6002b8f871.png)

![](../../assets/85f53c05325a77b8.png)
...
![](../../assets/c9e5bd2a9797e5b9.png)

如果感兴趣可以看下这篇文章

https://bbs.huaweicloud.com/blogs/348729?utm\_source=oschina&utm\_medium=bbs-ex&utm\_campaign=other&utm\_content=content
